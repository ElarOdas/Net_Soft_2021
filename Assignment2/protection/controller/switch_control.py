...
class SwitchControl:
    ...
    def writeTunnelrule(self, dstAddr, dstPrefix, dstAddrSwitch, dstMac, port, protection):
        # Assemble the table entry
        table_entry = self.p4info_helper.buildTableEntry(
            table_name="MyIngress.ipv4_lpm",
            match_fields={
    ...
    def writeDeTunnelrule(self, dstAddr):
        # Assemble the table entry
        table_entry = self.p4info_helper.buildTableEntry(
            table_name="MyIngress.detunnel_exact",
            match_fields={
    ...
    def writeCloneRule(self, dstAddr, dstMac, port):
        # Assemble the table entry
        table_entry = self.p4info_helper.buildTableEntry(
            table_name="MyEgress.oneplusone",
            match_fields={
                "hdr.ipv4.dstAddr": dstAddr
            },
            action_name="MyEgress.cloneProtection",
            action_params={
                "dstAddr": dstMac,
                "port": port
            })
        # Write the table entry
        self.switch.WriteTableEntry(table_entry)
        print "Installed Clone rule on switch %s" % self.switch.name
